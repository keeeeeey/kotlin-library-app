...
open class BookServiceTest @Autowired constructor(
    ...
    fun saveBook() {
        // given
        val boolRequest = BookRequest("이상한 나라의 엘리스", BookType.COMPUTER);

        // when
        bookService.saveBook(boolRequest)
    ...
    fun returnBookTest() {
        // given
        bookRepository.save(Book.fixture("이상한 나라의 엘리스"))
        userRepository.save(User("김기윤", null))
        val loanRequest = BookLoanRequest("김기윤", "이상한 나라의 엘리스")
        bookService.loanBook(loanRequest)

        // when
        val request = BookReturnRequest("김기윤", "이상한 나라의 엘리스")
